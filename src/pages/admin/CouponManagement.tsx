    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 space-y-6">
          <div className={`${isDark ? 'bg-[#1a1a1a]' : 'bg-white'} rounded-3xl border ${isDark ? 'border-white/10' : 'border-slate-100'} shadow-sm overflow-hidden animate-in fade-in duration-300`}>
            <div className={`p-4 border-b ${isDark ? 'border-white/5' : 'border-slate-50'} ${isDark ? 'bg-white/5' : 'bg-slate-50/50'}`}>
                    ...
                    filteredCoupons.map((coupon) => {
                      ...
                      return (
                        <tr 
                          ...
                          className={cn(
                            `${isDark ? 'hover:bg-white/5' : 'hover:bg-slate-50/50'} transition-colors cursor-pointer group border-l-4`,
                            isSelected 
                              ? `${isDark ? 'bg-[#FFC928]/5 border-l-[#FFC928]' : 'bg-amber-50/20 border-l-[#FFC928]'}`
                              : "border-l-transparent"
                          )}
                        >
                          <td className="py-4 px-6">
                            <div className="flex items-center gap-3">
                              <div className={`w-10 h-10 rounded-xl ${isDark ? 'bg-white/10' : 'bg-orange-50'} flex items-center justify-center ${isDark ? 'text-[#FFC928]' : 'text-orange-500'}`}>
                                <Ticket size={20} />
                              </div>
                              <span className={`font-black ${isDark ? 'text-white' : 'text-slate-900'} tracking-tight`}>{coupon.code}</span>
                            </div>
                          </td>
                          <td className="py-4 px-6">
                          ...
                          <td className="py-4 px-6">
                            ...
                            >
                              <div className={cn(
                                ...
                                coupon.isActive ? "bg-green-500" : `${isDark ? 'bg-white/20' : 'bg-slate-300'}`
                              )}>
                                <div className={cn(
        ...
        <div className="xl:col-span-1">
          {selectedCoupon ? (
            <div className={`${isDark ? 'bg-[#1a1a1a] border-white/10' : 'bg-white border-slate-100'} rounded-3xl border shadow-sm p-6 space-y-6 animate-in fade-in duration-300`}>
              <div>
                <div className="flex justify-between items-start">
                  ...
                  <div className={`flex items-center gap-2 ${isDark ? 'bg-white/5 border-white/10' : 'bg-slate-50 border-slate-100'} p-1.5 px-3 rounded-xl border shadow-sm`}>
                    <button
                      type="button"
                      onClick={() => handleToggleStatus(selectedCoupon)}
                      className={cn(
                        "w-9 h-5 rounded-full transition-all relative",
                        selectedCoupon.isActive ? "bg-green-500" : `${isDark ? 'bg-white/20' : 'bg-slate-300'}`
                      )}
                    >
                      <div className={cn(
                        "absolute top-0.5 w-4 h-4 rounded-full bg-white transition-all shadow-sm",
                ...
                <div className="max-h-[320px] overflow-y-auto space-y-2.5 pr-1 no-scrollbar">
                  {loadingSelectedHistory ? (
                    ...
                    selectedUsageHistory.map((order) => (
                      <div 
                        key={order.id} 
                        className={`p-3 ${isDark ? 'bg-white/5 border-white/10' : 'bg-slate-50/60 border-slate-100/40'} border rounded-xl flex items-center justify-between`}
                      >
                        <div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      <Ticket className={cn(
                        ...
                        codeError ? "text-red-400" : formData.code && !codeError ? "text-green-500" : `${isDark ? "text-slate-500" : "text-slate-400"}`
                      )} size={18} />
                      <input
                        type="text"
                        placeholder="EX: PROMO20"
                        className={cn(
                          `w-full pl-12 pr-4 h-12 ${isDark ? 'bg-[#111] text-white' : 'bg-slate-50 text-slate-900'} border rounded-2xl text-sm font-black uppercase tracking-widest focus:ring-2 transition-all outline-none`,
                          isDark ? "focus:bg-[#111]" : "focus:bg-white",
                          codeError 
                            ? "border-red-200 focus:ring-red-500/10 focus:border-red-500" 
                            : formData.code && !codeError
                              ? "border-green-200 focus:ring-green-500/10 focus:border-green-500"
                              : `${isDark ? 'border-white/10' : 'border-slate-100'} focus:ring-orange-500/20 focus:border-orange-500`
                        )}
                        value={formData.code}
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      ...
                      <input
                        type="date"
                        className={`w-full pl-12 pr-4 h-12 ${isDark ? 'bg-[#111] border-white/10 text-white' : 'bg-slate-50 border-slate-100 text-slate-900'} border rounded-2xl text-sm font-bold focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all outline-none`}
                  ...
                  <div className="flex items-center gap-3 pt-6 px-1">
                    <button
                      ...
                      className={cn(
                        ...
                        formData.isActive ? "bg-green-500" : `${isDark ? 'bg-white/20' : 'bg-slate-300'}`
                      )}
                    >
                      <div className={cn(
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        formData.targetAudience === opt.value
                          ? `${isDark ? 'bg-[#FFC928]/10 border-[#FFC928]/30 text-[#FFC928]' : 'bg-orange-50 border-orange-200 text-orange-700'}`
                          : `${isDark ? 'bg-white/5 border-white/10 text-slate-500 hover:border-white/20' : 'bg-slate-50 border-slate-100 text-slate-400 hover:border-slate-200'}`
                      )}
                    >
                      {opt.label}
                    </button>
                  ...
                  <div className={`grid grid-cols-2 gap-3 ${isDark ? 'bg-white/5' : 'bg-slate-50'} rounded-2xl p-4`}>
                    <div>
                      ...
                      <input
                        ...
                        step="0.5"
                        placeholder="4.0"
                        className={`w-full px-3 h-10 ${isDark ? 'bg-[#111] border-white/10 text-white' : 'bg-white border-slate-100 text-slate-900'} border rounded-xl text-sm font-bold focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500`}
...
<div className="text-center py-12">
                ...
                <div className="space-y-3">
                  {usageHistory.map((order) => (
                    <div 
                      key={order.id} 
                      className={`p-4 ${isDark ? 'bg-white/5 border-white/10 hover:bg-white/10' : 'bg-slate-50/50 border-slate-100 hover:bg-white'} border rounded-2xl flex items-center justify-between group hover:shadow-md transition-all`}
                    >
                      <div className="flex items-center gap-4">
                        <div className={`w-10 h-10 ${isDark ? 'bg-white/10 text-slate-500' : 'bg-white text-slate-400'} rounded-xl flex items-center justify-center shadow-sm`}>
                          <User size={18} />
                        </div>
                        <div>
                          <p className={`font-black ${isDark ? 'text-white' : 'text-slate-900'} text-sm`}>{order.customerName}</p>
                          <p className={`text-[10px] font-bold ${isDark ? 'text-slate-500' : 'text-slate-400'} uppercase tracking-widest`}>{order.customerPhone}</p>
                        </div>
                       </div>
            ...
            <div className={`p-8 border-t ${isDark ? 'border-white/5 bg-white/5' : 'border-slate-50 bg-slate-50/30'}`}>
              <Button 
                onClick={() => setIsHistoryModalOpen(false)} 
